...
pubDatetime: 2026-09-23T18:00:00+02:00
title: "Faster substructure search with roaring bitmaps and an inverted index"
tags:
  - cheminformatics
  - RDKit
  - substructure search
  - roaring bitmaps
  - benchmarking
description: Benchmarking naive scans, pattern fingerprints, roaring bitmaps, PostgreSQL, and inverted indexes for matching SMARTS queries against molecules.
---

One of the most insightful talks at [RDKit UGM 2026](https://github.com/rdkit/UGM_2026) was [Maciej Wójcikowski](https://www.linkedin.com/in/mwojcikowski/)'s talk about using roaring bitmaps to speed up substructure searches. Having some time in the evening after the UGM and during the hackathon, I decided to take a closer look.

Imagine you have a large corpus of SMARTS substructures (>50k) and want to find which ones are present in a molecule. Maciej talked about this approach being used in Merck's [Synthia](https://www.sigmaaldrich.com/DE/en/services/software-and-digital-platforms/synthia-retrosynthesis-software) retrosynthesis software, which makes sense if you have reaction templates and want to know which ones match the starting molecule. For my benchmark, I used the `uspto-grants` subset of [`ord-data`](https://github.com/open-reaction-database/ord-data): 1.77 million chemical reactions mined from United States patent grants. These reactions are also atom-mapped, so we can use AstraZeneca's [`reaction-utils`](https://github.com/MolecularAI/reaction_utils) library to get reasonably good templates. I sampled 50,000 unique templates as substructures and 1,000 unique reactants for the searches.

The benchmark code is available in the [roaring-substructure repository](https://github.com/asiomchen/roaring-substructure).

![One reactant structure and six matching query structures from the benchmark data.](./roaring-substructure-assets/reactant_ord_b3ca7045_matches.svg)

_One of the reactants used in the benchmark, shown with six matching substructures._
...
Here comes the main topic: roaring bitmaps. At the UGM, only Andrew Dalke raised his hand when asked whether anyone had experience with them. Apparently, more exotic data structures are not that common in the cheminformatics world.
...
Adding more CPUs alone does not make the search faster. For the 50k queries used here, the Postgres planner refused to start multiple workers for `Parallel Bitmap Heap Scan`, so both the index scan and recheck were sequential. But using 4 CPUs and 10 connections brought the search down to **1.6 seconds**—the fastest so far, though _slightly cheating_ because we're entering multiprocessing territory.
...
1. Rank the pattern fingerprint bits by how many queries contain them.
1. Walk that ranking and select up to 128 bits absent from the target molecule's fingerprint (the default limit).
1. Union the postings for those absent bits and skip every query they contain: a query requiring a missing bit cannot match.
1. Run the full fingerprint subset check and exact substructure match on the remaining queries.
